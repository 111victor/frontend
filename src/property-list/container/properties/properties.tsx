...
const Properties = () => {
    ...
    const filteredProperties = propertyDescriptions.filter((property: PropertyDescription) => {
        return (
            (!filters.propertyType.length || filters.propertyType.includes(property.property_type)) &&
            ...
            <div className="container">
                ...
                <div className="row">
                    {
                        filteredProperties.map((propertyDescription: PropertyDescription) => (
                            <div className="col-sm-3 py-2"  key={propertyDescription.id}>
                                <Link to={propertyDescription.id.toString()} state={{search: `?${searchParams.toString()}`}}>
                ...
                <nav aria-label="Page navigation example">
                    <ul className="pagination">
                        <li onClick = {() => fetchNewProperties(nextPageNumber === 1 ? 1 : nextPageNumber - 1 )} className={`page-item ${nextPageNumber === 1 ? 'disabled' : ''}` }><a className="page-link">Previous</a></li>
                        <li onClick = {() => fetchNewProperties(nextPageNumber + 1)} className="page-item"><a className="page-link">Next</a></li>
                    </ul>
                </nav>
            </div>
        </>
